...
void UC8119::do_full_refresh_() {
  ESP_LOGI(TAG, "Full refresh (ghost clear)");
...
void UC8119::dump_config() {
  ESP_LOGCONFIG(TAG, "UC8119 EPD Segment Driver:");
  ESP_LOGCONFIG(TAG, "  Address: 0x%02X", this->address_);
  ESP_LOGCONFIG(TAG, "  Framebuffer: %d bytes (%d segments)", FB_DATA_SIZE, FB_DATA_SIZE * 8);
  ESP_LOGCONFIG(TAG, "  Ghost clear interval: %u min", this->ghost_clear_interval_ms_ / 60000);
